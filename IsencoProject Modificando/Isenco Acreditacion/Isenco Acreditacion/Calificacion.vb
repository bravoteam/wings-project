Public Class Calificacion
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        SQLSTR = " SELECT NOMBRE,idmateria FROM MATERIAS WHERE IDCARRERA='" & IDLICE & "' AND IDESPECIALIDAD='" & IDESPE & "' " & _
            " AND IDSEMESTRE='" & cmbSemestre.Text & "' AND activa='TRUE' or nombre='servicio social interno'or " & _
            "nombre='actividades culturales y deportivas' ORDER BY TIPOCHECK DESC,IDMATERIA"
        SQLCmd.CommandText = SQLSTR 'Sets the SQL String
        SQLdr = SQLCmd.ExecuteReader 'Gets Data
        ...
        While SQLdr.Read
            ...
            Else
                ...
                DG.Columns.Add("porcentaje1", "% 1P ASISTENCIA")
                DG.Columns.Item("porcentaje1").Width = 80

                DG.Columns.Add("porcentaje2", "% 2P ASISTENCIA")
                DG.Columns.Item("porcentaje2").Width = 80
        ...
        For I = 0 To DG.RowCount - 2
            For J = 4 To DG.ColumnCount - 1
                ...
                SQLSTR = "SELECT CALIFICACION,calificacion2,asistenciaP1,asistenciaP2 FROM calificaciones WHERE matricula='" & DG(0, I).Value & "' AND  " & _
                " idmateria='" & IDmat & "' AND idsemestre='" & cmbSemestre.Text & "' AND idlicenciatura='" & IDLICE & "' and idgrupo='" & cmbGrupo.Text & "' " & _
                " GROUP BY idmateria,idsemestre,idlicenciatura,idgrupo,CALIFICACION,calificacion2,asistenciaP1,asistenciaP2"
                SQLCmd.CommandText = SQLSTR 'Sets the SQL String
                SQLdr = SQLCmd.ExecuteReader 'Gets Data
                While SQLdr.Read
                    ...
                    Else
                        ...
                        J = J + 1
                        DG(J, I).Value = SQLdr("asistenciaP1").ToString
                        J = J + 1
                        DG(J, I).Value = SQLdr("asistenciaP2").ToString
                    End If
                    contX = contX + 1
                End While
                SQLdr.Close()
            ...
            For j = 4 To DG.ColumnCount - 1
                ...
                If encontrado Then ' si encuentra el registro en la tabla de calificaciones solamente actualiza datos
                    ...
                    SQLCmd.CommandText = "UPDATE calificaciones SET calificacion = '" & cal & "', calificacion2 = '" & cal2 & "', " & _
                        "asistenciaP1= '" & P1 & "',asistenciaP2='" & P2 & "' WHERE matricula = '" & DG(0, i).Value & "'" & _
                ...
                Else 'sino lo encuentra entonces crea el registro de calificaciones de ese alumno
                    SQLCmd.CommandText = "insert into calificaciones (matricula,idmateria,idparcial,idsemestre,idlicenciatura," & _
                        " idgrupo,calificacion,calificacion2,asistenciaP1,asistenciaP2,IDMAESTRO) values ('" & DG(0, i).Value & "','" & IDmat & "','1','" & cmbSemestre.Text & "'," & _
                        "'" & IDLICE & "','" & cmbGrupo.Text & "','" & cal & "','" & cal2 & "','" & P1 & "','" & P2 & "','1')"
                End If
                SQLCmd.ExecuteNonQuery() 'Gets Data
    ...
    Private Sub cargar_combos()
        ...
        SQLSTR = "SELECT NUMERO FROM semestres ORDER BY NUMERO asc"
        SQLCmd.CommandText = SQLSTR 'Sets the SQL String
        SQLdr = SQLCmd.ExecuteReader 'Gets Data
        While SQLdr.Read() 'While Data is Present
            cmbSemestre.Items.Add((SQLdr("NUMERO").ToString).ToUpper)
        End While
        SQLdr.Close()
