...
Namespace Modelos
    ...
    Public Class AlumnosModel
        ...
        Public Sub truncarAlumno(ByVal Matricula As String, ByVal Nombre As String, ByVal ApellidoPaterno As String, ByVal ApellidoMaterno As String, _
            ...
            b.cerrarConexion()
        End Sub
        Public Sub truncarAlumno2(ByVal Matricula As String, ByVal Nombre As String, ByVal ApellidoPaterno As String, ByVal ApellidoMaterno As String, _
                                ByVal Campus As String, ByVal Licenciatura As String, ByVal Especialidad As String, _
                                ByVal Modalidad As String, ByVal Turno As String, ByVal Semestre As String)
            ' se agrego la validacion de try-catch y tambien se agrego la transaccion para validar de que si existe un error se deshaga la operacion
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
        ...
        Public Sub altaAlumno(ByVal Nombre As String, ByVal ApellidoPaterno As String, ByVal ApellidoMaterno As String, _
            ...
            ByVal puntajeceneval As String, ByVal promediobachillerato As String, ByVal BACHILLERATOPROCEDENCIA As String)

            Dim b As New BaseDatos
            Dim sql As String
            '------------CHECAR QUE EL ALUMNO NO EXISTA CON 
            sql = "INSERT INTO alumno ([NOMBRE], [APELLIDO_PATERNO], [APELLIDO_MATERNO], [GENERO], [CALLE], " & _
                "[NUMERO_EXTERIOR], [NUMERO_INTERIOR], [COLONIA],[IDMUNICIPIO],[IDESTADO],[CODIGO_POSTAL],[CURP], " & _
                "[TELEFONO_CASA],[TELEFONO_CEL],[CORREO_ELECTRONICO],[FECHA_NACIMIENTO],[FECHA_INGRESO],[FECHA_SALIDA], " & _
                "[TIPO_SANGRE],[OBSERVACIONES],[CONTACTO_EMERGENCIA],[TELEFONO_EMERGENCIA],[MATRICULA],[IDCAMPUS],[IDturno], " & _
                "[IDSTATUS],[IDAREA],[IDMODALIDAD],[IDESPECIALIDAD],[IDSEMESTRE],[IDGRUPO],[IDLICENCIATURA],[FOTO],[PLANEST], " & _
                "[GENERACION],[BECA],[NO_SS],[PUNTAJECENEVAL],[PROMEDIOBACHILLERATO],[BACHILLERATOPROCEDENCIA]) " & _
                "VALUES('" & b.SafeSqlLiteral(Nombre) & "', " & _
                ...
                "'" & b.SafeSqlLiteral(puntajeceneval) & "', " & _
                "'" & b.SafeSqlLiteral(promediobachillerato) & "', " & _
                "'" & b.SafeSqlLiteral(BACHILLERATOPROCEDENCIA) & "');"
            b.abrirConexion()
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                For Each F As DataRow In ds.Tables(0).Rows
                    A = New Alumno(F("IDALUMNO").ToString,
                                   ...
                                   F("PUNTAJECENEVAL").ToString,
                                   F("PROMEDIOBACHILLERATO").ToString,
                                   F("BACHILLERATOPROCEDENCIA").ToString)
                Next
            Else
                Return Nothing
            End If
        ...
        Public Function getMaestros() As List(Of String)
            ...
            Dim ds As New DataSet
            Dim sql As String
            sql = "select nombre,apellido_paterno,apellido_materno from maestro order by nombre,apellido_paterno,apellido_materno"
            b.abrirConexion()
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
            ...
            Return Maestros
        End Function
        Public Function getcicloescolar() As List(Of String)
            Dim cicloescolar As New List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            sql = "select descripcion from ciclo_escolar order by descripcion"
            b.abrirConexion()
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    cicloescolar.Add(f(0).ToString)
                Next
            End If
            Return cicloescolar
        End Function
        Public Function getAdministrativos() As List(Of String)
            Dim administrativos As New List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            sql = "select nombre from directivos order by nombre"
            b.abrirConexion()
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    administrativos.Add(f(0).ToString)
                Next
            End If
        ...
        Public Function getCampus() As List(Of String)
            ...
            b.abrirConexion()
            sql = "SELECT MUNICIPIO AS 'CAMPUS' " & _
                "FROM CAMPUS ORDER BY MUNICIPIO;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
        ...
        Public Function getLicenciaturas() As List(Of String)
            ...
            b.abrirConexion()
            sql = "SELECT UPPER(NOMBRE) AS LICENCIATURA " & _
                "FROM licenciaturas ORDER BY LICENCIATURA;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
        ...
        End Function
        Public Function getBachilleratos() As List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            Dim Bachilleratos As New List(Of String)

            b.abrirConexion()
            sql = "SELECT UPPER(NOMBRE) AS NOMBRE " & _
                "FROM bachilleratos ORDER BY NOMBRE;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    Bachilleratos.Add(f("NOMBRE").ToString)
                Next
            Else
                Bachilleratos = Nothing
            End If

            Return Bachilleratos
        End Function
        Public Function getSecundarias() As List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            Dim Secundarias As New List(Of String)

            b.abrirConexion()
            sql = "SELECT UPPER(NOMBRE) AS NOMBRE " & _
                "FROM secundarias ORDER BY NOMBRE;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    Secundarias.Add(f("NOMBRE").ToString)
                Next
            Else
                Secundarias = Nothing
            End If
            ...
            Return Secundarias
        End Function
        Public Function getBecas() As List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            Dim becas As New List(Of String)

            b.abrirConexion()
            sql = "SELECT UPPER(descripcion) AS NOMBRE " & _
                "FROM becas ORDER BY NOMBRE;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    becas.Add(f("NOMBRE").ToString)
                Next
            Else
                becas = Nothing
            End If
            ...
            Return becas
        End Function
        Public Function getDiscapacidades() As List(Of String)
            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            Dim discapacidades As New List(Of String)

            b.abrirConexion()
            sql = "SELECT UPPER(descripcion) AS NOMBRE " & _
                "FROM discapacidades ORDER BY NOMBRE;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    discapacidades.Add(f("NOMBRE").ToString)
                Next
            Else
                discapacidades = Nothing
            End If
        ...
        Public Function getGrupos() As List(Of String)
            ...
            b.abrirConexion()
            sql = "SELECT DISTINCT(NOMBRE) AS GRUPO FROM GRUPOS " & _
                "ORDER BY GRUPO;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
        ...
        Public Function getEspecialidades() As List(Of String)
            ...
            sql = "select upper(descripcion) from especialidades ORDER BY DESCRIPCION"
            ds = b.getDataSet(sql)
        ...
        End Function
        Public Function getClinicas() As List(Of String)

            Dim b As New BaseDatos
            Dim ds As New DataSet
            Dim sql As String
            Dim clinicas As New List(Of String)

            b.abrirConexion()
            sql = "SELECT nombre FROM clinicas " & _
                "ORDER BY nombre;"
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    clinicas.Add(f("nombre").ToString)
                Next
            Else
                clinicas = Nothing
            End If
        ...
        Public Function getMaterias(ByVal semestre As String, ByVal idlicenciatura As String) As List(Of String)
            ...
            Dim ds As New DataSet
            Dim sql As String
            sql = "select nombre from materias where idsemestre = '" & semestre & "' and idcarrera = '" & idlicenciatura & "' " & _
                "and activa=1 order by nombre"
            b.abrirConexion()
            ds = b.getDataSet(sql)
            b.cerrarConexion()
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Public Class Alumno
        ...
        Public Sub New(ByVal IdAlumno As String, _
                        ...
                        ByVal NoSS As String, _
                        ByVal AdeudoLibrosMaterial As String, _
                        ByVal PuntajeCeneval As String, _
                        ByVal PromedioBachillerato As String, _
                        ByVal BACHILLERATOPROCEDENCIA As String)
            ...
            Me.PuntajeCeneval = PuntajeCeneval
            Me.PromedioBachillerato = PromedioBachillerato
            Me.BACHILLERATOPROCEDENCIA = BACHILLERATOPROCEDENCIA
        End Sub
